...
if(!empty($_POST['htno']) && !empty($_POST['stname']) && !empty($_POST['nationality']) && !empty($_POST['doj']) && !empty($_POST['dob']) && !empty($_POST['year']) && !empty($_POST['course']) && !empty($_POST['spec']) && !empty($_POST['religion'])&& !empty($_POST['caste']) || !empty($_POST['castename'])
	&& !empty($_POST['qualify'])&& !empty($_POST['pay'])&& !empty($_POST['medical']) && !empty($_POST['ldate'])){

$res = $_POST;
$date = $res['ldate'];
$res['ldate'] = date("d/m/Y", strtotime($date));
$_SESSION['caste']=$res['casteedit'];
  require('header.php');
            ...
            <form class="form-horizontal" role="form" action="tc4.php" method="post">
                  ...
                  <div class="form-group">
                   ...
                   <?php
											...
											$ldate=$res['ldate'];
											$actdate= $ldate[3].$ldate[4].$ldate[2].$ldate[0].$ldate[1].$ldate[5].$ldate[6].$ldate[7].$ldate[8].$ldate[9];
											echo date("F / Y",strtotime($actdate));
											$_SESSION['ledate'] = date("F / Y",strtotime($actdate));
				   ...
				   <input type='hidden' name='pay' value='<?php echo $res['pay']; ?>' />
				    <input type='hidden' name='medical' value='<?php echo $res['medical']; ?>' />
					 <input type='hidden' name='ldate' value='<?php echo $res['ldate']; ?>' />
					  <input type='hidden' name='appdate' value='<?php echo $today = date("F / Y "); ?>' />
                  <input type='hidden' name='nationality' value='<?php echo $res['nationality']?>' />
